        ...
        with self._connection.cursor() as cur:
            cur.execute(
                ...
                (entry.id, entry.timestamp, entry.operation,
                 entry.conversation_id, entry.participant_id,
                 entry.provider, entry.model, entry.success,
                 entry.error_message, entry.user_id,
        ...
        with self._connection.cursor() as cur:
            cur.execute(
                ...
                SELECT id, timestamp, operation, conversation_id, participant_id,
                       provider, model, success, error_message, user_id,
                       input_tokens, output_tokens
                FROM audit_log WHERE conversation_id = %s ORDER BY timestamp
                """,
                (conversation_id,),
            )
            rows = cur.fetchall()
        ...
        with self._connection.cursor() as cur:
            cur.execute(
                ...
                SELECT id, timestamp, operation, conversation_id, participant_id,
                       provider, model, success, error_message, user_id,
                       input_tokens, output_tokens
                FROM audit_log WHERE timestamp >= %s AND timestamp < %s ORDER BY timestamp
                """,
                (start, end),
            )
            rows = cur.fetchall()
