...
**Basis:** Empirische Beobachtungen aus 7 Durchlaeufen (01.–04.04.2026)

---

## Das Wichtigste zuerst: Token-Explosion ist real

Bei sequentieller Orchestrierung waechst der Kontext quadratisch.
...
Aufruf — nur fuer den Kontext.

**Drei Agenten sind nicht 50% teurer als zwei. Sie sind 100% teurer.**

Jede zusaetzliche Antwort vergroessert den Kontext fuer alle
folgenden Aufrufe. Das fuehrt zu:
- API-Rate-Limits bei langen Sessions
- Abnehmender Antwortqualitaet (Kontext ueberflutet den Prompt)
- Unvermeidlichem Abbruch bei >30.000 Tokens pro Aufruf

---

## Fuenf Setups, fuenf Ergebnisse
...
**Empfehlung:** Das beste Setup fuer kreative und konzeptionelle
...
- **Ergebnis:** Substanzloser Plan ohne Bezug zur Realitaet

**Was schiefging:**
1. 55% der Nachrichten gingen fuer Setup drauf (Wer ist da?
...
- "openai" → Chat Completions API (tatsaechlich: Responses API)
- "docker-compose" → postgres:16-alpine (tatsaechlich: postgres:17)
- Alle drei Agenten zeigten dasselbe Verhalten

**Lektion:** Chat-Agents im API-Paradigma koennen Code nicht
zuverlaessig analysieren. Fuer Code-Aufgaben braucht man
einen Verifikator.
...
**Empfehlung:** Das beste Setup fuer Code-Analyse, technische
Bewertungen und alles wo Fakten geprueft werden muessen.
...
**Empfehlung:** Fuer Einzelaufgaben (Security Audit, Architektur-Review)
ist das Web-Interface oft besser als Conclave — weil es den Code
tatsaechlich liest. Conclave glänzt bei der Orchestrierung
...
**Nie eine Endlos-Conversation fuer mehrere Aufgaben.**

```python
# SCHLECHT: Alles in einer Conversation
conv = conclave.create_conversation("Refactoring")
# ... 117 Nachrichten spaeter: Token-Limit, substanzloses Ergebnis
...
### 2. Parallel fuer Analyse, sequentiell fuer Diskussion

```python
# ANALYSE: Jeder liest unabhaengig
result = conclave.orchestrate(conv_id, ["Claude", "GPT", "Gemini"],
                               parallel=True)

# DISKUSSION: Jeder sieht die Vorgaenger
result = conclave.orchestrate(conv_id, ["Claude", "GPT", "Gemini"],
                               parallel=False)
```
...
Der dritte Agent hat in keinem Durchlauf einen eigenstaendigen
...
Agenten die sich gegenseitig bestaetigen.

Wenn du drei brauchst, gib dem dritten eine klar abgegrenzte
Aufgabe (z.B. "fasse zusammen und priorisiere"), nicht eine
Doppelrolle die mit den anderen ueberlappt.

### 4. Belegpflicht einfuehren

```
SCHLECHT: "Die Persistenzschicht hat Luecken."
...
```
SCHLECHT: "Nenne Befunde ueber den Code."
GUT:      "Formuliere Vermutungen. Schreibe 'ich vermute'
           statt 'es ist so'. Fuer jede Vermutung: formuliere
           eine Prueffrage."
```

Im Hypothesen-Modus lag GPT bei 100%, Claude bei 17%.
Im Fakten-Modus lagen alle bei ~0%. Das Wort "ich vermute"
aendert den Generierungsmodus des Modells fundamental.

### 6. Antwortlaenge begrenzen

```
PROMPT: "Maximale Antwortlaenge: 15 Zeilen. Zustimmung ohne
         neuen Inhalt = 'Einverstanden.' Ein Wort."
```

Das eliminiert Zeremonie ("Brillant! Ich stimme vollkommen zu
und moechte hinzufuegen...") und spart Token. Wenn ein Agent
nur 15 Zeilen hat, muss er sich entscheiden ob er sie fuer
Hoeflichkeit oder fuer Inhalt verwendet.

### 7. Setup nicht in der Conversation verhandeln

Rollen, Regeln, Workspace-Pfade — alles was sich nicht aendert
gehoert in den ersten Prompt oder in die Chat-Rules, nicht in
...
conclave.send_message(conv_id, "Wer moechte welche Rolle?")
...
Faktor 150x weniger Tokens fuer bessere Ergebnisse.
...
| Aufgabe | Empfehlung |
|---------|-----------|
| Texte schreiben/ueberarbeiten | Conclave (2 Agenten, aktiver Moderator) |
...
| Architektur-Review | Web-Interface direkt, dann Conclave fuer Diskussion |
| Schnelle Einzelfrage | Kein Conclave noetig |
...
4. **API vs. Web:** Muss der Agent Code lesen? → Web-Interface ist zuverlaessiger

> Chat-Agents koennen nicht lesen, aber sie koennen fragen.
> Tool-Use-Agents koennen lesen, aber sie brauchen Fragen.
> Web-Agents mit Extended Thinking koennen beides —
