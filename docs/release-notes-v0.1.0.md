...
Conclave v0.1.0 ist ein lokales Multi-Agent-Arbeitswerkzeug fuer einzelne
Nutzer. Es bringt mehrere KI-Modelle in strukturierte Conversations, gibt ihnen
explizite Rollen und macht ihre Arbeit ueber Runs nachvollziehbar.
...
- Lokale Workspace-Grenzen mit Hidden-Policy und Dateigroessenlimits.
- Runs und UsageRecords fuer Verlauf, Fehler, Dauer und Token-Nutzung.
- SQLite-Migration aus bestehenden lokalen Installationen.
- CI-Matrix fuer Windows und Ubuntu mit Python 3.11 und 3.12.
- Release-Artefakte als Wheel und Source Distribution.

## Installation

Der Zielpfad nach Veroeffentlichung ist:
...
Vor der PyPI-Veroeffentlichung kann das gebaute Wheel installiert werden:
...
Remote-Provider erhalten die fuer den Modellaufruf erforderlichen
Prompt-/Kontextdaten. Ollama und lokale kompatible Endpunkte koennen
vollstaendig lokal betrieben werden.
...
- Lokal verschluesselte Agent-Provider-Secrets.
...
- Remote-Provider uebertragen Prompt-/Kontextdaten extern.
...
Gegenueber dem alten Enterprise-Stand wurden entfernt:

- Consent-Management.
- DPA-/AV-Vertragsverwaltung.
- DSGVO-Lifecycle-UI.
- Compliance-zentrierte API- und CLI-Oberflaechen.

Bestehende lokale SQLite-Daten koennen explizit migriert werden:
...
- CLI-Hilfe enthaelt `migrate-personal`, aber keine Consent-/DPA-Kommandos.
