...
**Auslöser:** POST /conversations/{id}/participants/{pid}/invoke
...
1. **Authentifizierung** — `api/app.py:_authenticate()` prüft Bearer-Token oder X-API-Key. Bei RoleBasedAuthService zusätzlich `check_permission()`.
...
4. **Conversation laden** — `application/conversation_flow.py:load_conversation()` lädt Conversation, Participants und Messages aus den lokalen Repositories.
...
6. **Adapter holen** — `adapter_registry.py:get_for(participant_id)` liefert den registrierten Adapter oder baut ihn über den Lazy Builder aus der lokalen Agent-Konfiguration.
...
8. **Messages formatieren** — der jeweilige Adapter formatiert Conversation, Rollen und Modellparameter für den Provider.
...
12. **Agent-Direktiven verarbeiten** — `conversation_flow.py:_process_agent_directives()` schreibt `@save(...)`-Blöcke in den Workspace und ersetzt sie in der Antwort durch einen Workspace-Link.
...
15. **Audit-Log** — `conversation_flow.py:_audit()` ergänzt den technischen Verlauf für Usage-Auswertungen.
