...
**Ausloeser:** POST /conversations/{id}/orchestrate, /orchestrate-parallel, /auto-loop
...
3. `application/orchestrator.py:Orchestrator.run(conversation_id, sequence)` iteriert ueber Participants.
4. Pro Participant: `service.invoke_participant()` — jeder sieht die Antworten aller Vorgaenger.
5. Bei `ConversationNotFound` oder `AdapterNotFound`: Loop stoppt, Partial Result zurueck.
...
6. Gruppen untereinander sind sequentiell — spaetere Gruppen sehen fruehere Antworten.
7. Bei Exception in einem Agent: Loop stoppt, bisherige Responses + Fehlermeldung zurueck.
...
4. Nach jeder Antwort: Pruefung ob `stop_signal` (case-insensitiv) im Content enthalten.
5. Events:
